...
try {
  log.info('🔍 Analyse TypeScript complète...\n')
  ...
  if (errors.length === 0) {
    log.info('✅ Aucune erreur "never read" détectée!')
    ...
    log.info(`❌ ${errors.length} erreur(s) "never read" détectée(s):\n`)
  ...
  if (neverReadErrors.length > 0) {
    log.info(`❌ ${neverReadErrors.length} erreur(s) "never read" détectée(s):\n`)
    ...
    log.info(`❌ ${allErrors.length} erreur(s) TypeScript détectée(s):\n`)
    ...
    log.info('⚠️  Impossible de parser les erreurs. Output:')
    log.info(output.slice(0, 2000))
  }
}
